;; 
;; clms all.lisp creates the :clm-package, and :use's it ->
;; load all.lisp outside the :om-package to prevent name-clashes:

(in-package :cl-user)

(load "/home/andersvi/prosjekter/OM/OMCLM/OMCLM.git/omclm/sources/clm-5/all.lisp")

;; name-clashes with :om package :: RUN SOUND-SAMPLES SOUND-DURATION ENV FILTER
;; 
;; TODO: must be a better way...
;; 
;; macros expanding to clm:: calls, to avoid name clashes:

(defmacro clm-run (&rest args) `(clm::run ,@args))
(defmacro clm-sound-samples (&rest args) `(clm::sound-samples ,@args))
(defmacro clm-sound-duration (&rest args) `(clm::sound-duration ,@args))
(defmacro clm-env (&rest args) `(clm::env ,@args))
(defmacro clm-filter (&rest args) `(clm::filter ,@args))

;; exported symbols in sources/clm-5/export.lisp:

;; import symbols from :clm in to :om ::

(import '(
	  ;; above macros 
	  clm-run clm-sound-samples clm-sound-duration clm-env clm-filter

	  ;; the rest of clm's exported symbols
	  ...
	  make-filter
	  ;; filter ;; name-clash
	  filter? make-fir-filter fir-filter fir-filter? make-iir-filter iir-filter iir-filter?
	  ...
	  inverse-integrate
	  ;; env ;; name-clash
	  make-env env? restart-env env-interp
	  ...
	  ;; sound-duration ;; name-clash

	  sound-chans sound-comment sound-data-format sound-data-location sound-datum-size sound-maxamp
	  sound-length
	  ;; sound-samples ;; name-clash
	  sound-framples sound-srate sound-header-type sound-loop-info
